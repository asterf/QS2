...
1. Enterをｵｼﾃｸﾀﾞｻｲ
2. base32形式のシークレットシードを入力。
3. 次にハッシュアルゴリズムを入力(そんままスキップすればSHA-1で、通常はそれでおk)
4. ちょっとしたらなんか出てくると思います、それのコードをコピってくださいませ。
## ハッシュアルゴリズム・シークレットシードの調べ方
2FAの時に出てくるQRコードを読み取って、otpauth://から始まるテキストの、「algorithm=」から始まるのがアルゴリズム、「secret=」から始まるのがシード。
## Error内容
...
## 注意
- about:blankとかsecure contextじゃないところではcrypto APIが利用できないため動作しません。
- 問題が発生しても責任は負いません。
